...
class CameraFragment : Fragment() {
    ...
    /** Request builder linked with previewRequest CaptureRequest */
    private lateinit var mPreviewRequestBuilder : CaptureRequest.Builder

    /** Requests used for preview and recording in the [CameraCaptureSession] */
    private lateinit var recordRequest : CaptureRequest

    /** Builder for the recordRequest */
    ...
    private fun initializeCamera() = lifecycleScope.launch(Dispatchers.Main) {
        ...
        /** Preview builder initialization */
        mPreviewRequestBuilder = session.device.createCaptureRequest(CameraDevice.TEMPLATE_PREVIEW)
        mPreviewRequestBuilder.addTarget(viewFinder.holder.surface)

        /* Update the request with its builder */
        ...
        // We need to change the builder every time the flash turn off/on
        if(args.flash) {
            flashButton.setOnCheckedChangeListener{
                _, isChecked ->
                if(isChecked){
                    Log.i("DEBUG","Flash turned on !")
                    mPreviewRequestBuilder.set(CaptureRequest.FLASH_MODE, CaptureRequest.FLASH_MODE_TORCH)
                } else {
                    Log.i("DEBUG", "Flash turned off !")
            ...
            Log.i("DEBUG","Flash not available")
        }
        ...
        capture_button.setOnTouchListener { view, event ->
            when (event.action) {
                MotionEvent.ACTION_DOWN -> lifecycleScope.launch(Dispatchers.IO) {
                    ...
                    Log.i("DEBUG","flash button checked ? " + flashButton.isChecked())
                    // Adding flashlight to the recordRequest if requested
                    if (flashButton.isChecked()){
                        recordRequestBuilder.set(CaptureRequest.FLASH_MODE, CaptureRequest.FLASH_MODE_TORCH)
                    }
